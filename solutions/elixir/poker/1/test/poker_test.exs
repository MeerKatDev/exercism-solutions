defmodule PokerTest do
  ...
  test "multiple hands with the same high cards, tie compares next highest ranked, down to last card" do
    high_of_8_low_of_3 = ~w(3S 5H 6S 7H 8D)
  ...
  test "one pair beats high card" do
    ...
    assert Poker.best_hand([high_of_king, pair_of_4]) == [~w(2S 4H 4D 6S JH)]
  end
  ...
  test "highest pair wins" do
    ...
    assert Poker.best_hand([pair_of_2, pair_of_4]) == [~w(2S 4H 4D 6C JD)]
  end
  ...
  test "two pairs beats one pair" do
    ...
    assert Poker.best_hand([pair_of_8, fives_and_fours]) == [~w(4S 4C 5H 5C 8C)]
  end
  ...
  test "both hands have two pairs, highest ranked pair wins" do
    ...
    assert Poker.best_hand([eights_and_twos, fives_and_fours]) == [~w(2S 2D 3H 8H 8D)]
  end
  ...
  test "both hands have two pairs, with the same highest ranked pair, tie goes to low pair" do
    ...
    assert Poker.best_hand([queens_and_twos, queens_and_jacks]) == [~w(8D JD JS QH QC)]
  end
  ...
  test "both hands have two identically ranked pairs, tie goes to remaining card (kicker)" do
    ...
    assert Poker.best_hand([queens_jacks_and_8, queens_jacks_and_2]) == [~w(8D JD JS QH QC)]
  end
  ...
  test "three of a kind beats two pair" do
    ...
    assert Poker.best_hand([eights_and_twos, three_fours]) == [~w(4S 4C 4H 5H 8S)]
  end
  ...
  test "both hands have three of a kind, tie goes to highest ranked triplet" do
    ...
    assert Poker.best_hand([three_twos, three_aces]) == [~w(4S 8C AH AS AD)]
  end
  ...
  test "with multiple decks, two players can have same three of a kind, ties go to highest remaining cards" do
    ...
    assert Poker.best_hand([three_aces_7_high, three_aces_8_high]) == [~w(4S 8C AH AS AD)]
  ...
  test "a straight beats three of a kind" do
    ...
    assert Poker.best_hand([three_fours, straight]) == [Enum.sort(straight)]
  end
  ...
  test "aces can be start a straight (A 2 3 4 5)" do
    ...
    assert Poker.best_hand([three_fours, straight_to_5]) == [~w(2D 3S 4D 5C AH)]
  end
  ...
  test "both hands with a straight, tie goes to highest ranked card" do
    ...
    assert Poker.best_hand([straight_to_8, straight_to_9]) == [Enum.sort(straight_to_9)]
  end
  ...
  test "both hands have a flush, tie goes to high card, down to the last one if necessary" do
    ...
    flush_to_7 = ~w(2S 4S 5S 6S 7S)
    assert Poker.best_hand([flush_to_9, flush_to_7]) == [Enum.sort(flush_to_9)]

    flush_to_9_with_4_matches = ~w(3S 6S 7S 8S 9S)
    assert Poker.best_hand([flush_to_9, flush_to_9_with_4_matches]) == [Enum.sort(flush_to_9)]
  end
  ...
  test "full house beats a flush" do
    ...
    assert Poker.best_hand([flush_to_8, full]) == [~w(4S 4C 4H 5H 5D)]
  end
  ...
  test "four of a kind beats a full house" do
    ...
    assert Poker.best_hand([four_3s, full]) == [~w(2S 3S 3H 3D 3C)]
  end
  ...
  test "with multiple decks, both hands with identical four of a kind, tie determined by kicker" do
    ...
    assert Poker.best_hand([four_3s_and_2, four_3s_and_4]) == [~w(3S 3H 3D 3C 4S)]
  end

  @tag :pending
  test "straight flush beats four of a kind" do
    four_5s = ~w(4S 5H 5S 5D 5C)
    straight_flush_to_10 = ~w(6S 7S 8S 9S 10S)
