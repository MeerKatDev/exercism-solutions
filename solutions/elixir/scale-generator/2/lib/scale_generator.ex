defmodule ScaleGenerator do
  ...
  def chromatic_scale(tonic \\ "C"),
    do: tonic_from_scale(String.capitalize(tonic), @chromatic_scale)
  ...
  def flat_chromatic_scale(tonic \\ "C"),
    do: tonic_from_scale(String.capitalize(tonic), @flat_chromatic_scale)
  ...
  def scale(tonic, pattern) do
    ...
    |> Enum.reduce([tonic_uc], fn x, [h | _] = acc ->
      [step(scale, h, x) | acc]
    end)
    |> Enum.reverse()
  end
  ...
  defp tonic_from_scale(tonic, scale) do
    {a, b} = Enum.split_while(scale, &(&1 != tonic))
    b ++ a ++ [tonic]
  end
end
